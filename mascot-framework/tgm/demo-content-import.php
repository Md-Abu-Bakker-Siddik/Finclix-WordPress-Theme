...
class FinclixOneClickImport {
	...
	function __construct() {
		...
		//FTP
		$this->revSliderFolder_importURI = 'https://kodesolution.live/one-click-demo-import/finclix/import-rev-sliders/';
		$this->base_url = 'https://kodesolution.live/one-click-demo-import/finclix/v1.0/';

		//HTTP
		$this->preview_base_url = 'https://dev252.kodesolution.com/finclix/';
			...
			$final_output[] = array(
					...
					'import_redux'               => array(
							array(
									...
									'option_name' => 'finclix_redux_theme_opt',
							),
					),
					'import_preview_image_url'   => $current_url . 'screenshot.jpg',
					'import_notice'              => esc_html__( 'After you import this demo, you will have to import the slider separately. You will find slider in "import-sliders" folder in your downloaded package.', 'finclix' ),
	...
	public function after_import_setup( $selected_import ) {
		...
		finclix_generate_dynamic_css();
		finclix_generate_css_for_custom_theme_color_from_scss();
	...
	public function plugin_page_setup( $default_settings ) {
		...
		$default_settings['page_title']  = esc_html__( 'One Click Demo Import - ThemeMascot' , 'finclix' );
		$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'finclix' );
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								esc_html_e( 'Error while importing rev sliders', 'finclix' );
								exit;
							}
						}
					}

					esc_html_e( 'Rev sliders imported successfully', 'finclix' );
				} else {
					esc_html_e( 'RevSliderSliderImport instance is empty', 'finclix' );
				}

			} else {
				esc_html_e( 'RevSliderSliderImport class doesn\'t exist', 'finclix' );
			}

		} else {
			esc_html_e( 'Files don\'t exist', 'finclix' );
		}
	}

}

FinclixOneClickImport::get_instance();
