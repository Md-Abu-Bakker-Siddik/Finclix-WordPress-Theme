...
if( !class_exists( 'Finclix_Bootstrap_Comment_Walker' ) ) {
class Finclix_Bootstrap_Comment_Walker extends Walker_Comment {
	/**
				...
				<div id="div-comment-<?php comment_ID(); ?>">
					<div class="comment-body">
						<div class="comment-reply-link-wrapper">
							<ul class="comment-reply-link">
								<?php edit_comment_link( esc_html__( 'Edit', 'finclix' ), '<li class="list-inline-item edit-link">', '</li>' ); ?>
								...
								<?php
									comment_reply_link( array_merge( $args, array(
										...
										'reply_text'=> sprintf(esc_html__('Reply %s', 'finclix'), ''),
										'after'     => '</li>'
									) ) );
								?>
						...
						<div class="comment-metadata">
							<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
								<time datetime="<?php comment_time( 'c' ); ?>">
									<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'finclix' ), get_comment_date(), get_comment_time() ); ?>
								</time>
							</a>
						</div><!-- .comment-metadata -->

						<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation label label-info"><?php esc_html_e( 'Your comment is awaiting moderation.', 'finclix' ); ?></p>
						<?php endif; ?>
