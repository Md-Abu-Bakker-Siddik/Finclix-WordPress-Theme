...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Finclix_Breadcrumb_Trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Finclix_Breadcrumb_Trail.
 * @return void
 */
function finclix_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new Finclix_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
if( !class_exists( 'Finclix_Breadcrumb_Trail' ) ) {
class Finclix_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Finclix_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Finclix_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Finclix_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="breadcrumbs trail-items">',
			...
			foreach ( $this->items as $item ) {
				...
				if ( $item_count !== $item_position ) {
					//$angle_right = ' <span><i class="fa fa-angle-right"></i></span> ';
				}

				// Build the list item.
				$breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s%5$s</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta, $this->args['icon_html'] );
			}
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="breadcrumb-trail breadcrumbs-nav">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'finclix_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'finclix' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'finclix' ),
			'home'                => esc_html__( 'Home',                                  'finclix' ),
			'error_404'           => esc_html__( '404 Not Found',                         'finclix' ),
			'archives'            => esc_html__( 'Archives',                              'finclix' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'finclix' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'finclix' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'finclix' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'finclix' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'finclix' ),
	...
	protected function add_blog_items() {
		...
		//Override page title from theme option
		if( finclix_get_redux_option( 'blog-other-settings-show-blog-title-description', true ) ) {
			$title = finclix_get_redux_option( 'blog-other-settings-blog-title-text', finclix_redux_fallback_text_collection('blog') );
		}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'finclix' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'finclix' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'finclix' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'finclix' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'finclix' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'finclix' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'finclix' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'finclix' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'finclix' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'finclix' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'finclix' ) ) ) );
